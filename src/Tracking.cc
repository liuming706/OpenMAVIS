...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer,
  ...
  if (settings) {
    ...
    if (sensor == System::IMU_MONOCULAR || sensor == System::IMU_STEREO ||
        sensor == System::IMU_RGBD || sensor == System::IMU_MULTI) {
...
void Tracking::newParameterLoader(Settings* settings) {
  ...
  if (mSensor == System::IMU_MULTI) {
    mpORBextractorSideLeft = new ORBextractor(nFeatures * 2, fScaleFactor,
                                              nLevels, fIniThFAST, fMinThFAST);
    mpORBextractorSideRight = new ORBextractor(nFeatures * 2, fScaleFactor,
                                               nLevels, fIniThFAST, fMinThFAST);
  }
...
Sophus::SE3f Tracking::GrabImageMulti(const cv::Mat& imRectLeft,
                                      const cv::Mat& imRectRight,
  ...
  if (mSensor == System::IMU_MULTI)
    mCurrentFrame = Frame(
        mImGray, imGrayRight, imGraySideLeft, imGraySideRight, timestamp,
        mpORBextractorLeft, mpORBextractorRight, mpORBextractorSideLeft,
        mpORBextractorSideRight, mbleft, mbright, mbsideleft, mbsideright,
        mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2,
...
void Tracking::Track() {
  ...
  if (mState == NOT_INITIALIZED) {
    if (mSensor == System::STEREO || mSensor == System::RGBD ||
      ...
      MultiInitialization();
      std::cout << "MultiInitialization() done" << std::endl;
    } else {
    ...
    if (!mbOnlyTracking) {
      ...
      if (mState == OK) {
        ...
        if ((!mbVelocity && !pCurrentMap->isImuInitialized()) ||
          ...
          std::cout << "TrackReferenceKeyFrame(): " << bOK << std::endl;
        } else {
  ...
  for (int i = 0; i < mCurrentFrame.N; i++) {
    ...
    if (mCurrentFrame.mvpMapPoints[i]) {
      if (mCurrentFrame.mvbOutlier[i]) {
        ...
        else if (i < mCurrentFrame.Nleft + mCurrentFrame.Nright &&
                 i >= mCurrentFrame.Nleft)
          pMP->mbTrackInViewR = false;
        else if (i < mCurrentFrame.Nleft + mCurrentFrame.Nright +
                         mCurrentFrame.Nsideleft &&
...
bool Tracking::TrackLocalMap() {
  ...
  if (mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames &&
    ...
    std::cout << "mCurrentFrame.mnId " << mCurrentFrame.mnId
              ...
              << " + mMaxFrames && mnMatchesInliers " << mnMatchesInliers
              << " < 50 " << std::endl;
    return false;
  }
  ...
  if (mSensor == System::IMU_MONOCULAR) {
    if ((mnMatchesInliers < 15 && mpAtlas->isImuInitialized()) ||
      ...
      std::cout << "(mnMatchesInliers < 15 && mpAtlas->isImuInitialized()) || "
                   "(mnMatchesInliers < 50 && !mpAtlas->isImuInitialized())"
                << std::endl;
      return false;
    } else
    ...
    if (mnMatchesInliers < 15) {
      std::cout << "mnMatchesInliers < 15" << std::endl;
      return false;
    } else
      return true;
  } else {
    if (mnMatchesInliers < 30) {
      std::cout << "mnMatchesInliers < 30" << std::endl;
      return false;
    } else {
      return true;
    }
